...
class IceBarn(AbstractSolver):
    ...
    def configure(self, puzzle, init_symbol_grid):
        ...
        crossing_dir_pairs = sorted(set([tuple(sorted((v, puzzle.lattice_type.opposite_direction(v))))
        ...
        for p in puzzle.points:
            for (v, w), shift in zip(crossing_dir_pairs, shifts):
                if p in puzzle.shaded:
                    ...
                    sg.solver.add(Or(
                        sg.grid[shift(p)] == -1,
                        And(sg.grid.get(shift(p).translate(v)) == sg.grid[shift(p)] - 1,
                ...
                sg.solver.add(Or(sg.grid[shift(p)] == -1, sg.grid[shift(p)] == 1, Sum([
                    Or([sg.grid[other_shift(n.location)] == sg.grid[shift(p)] - 1 for other_shift in shifts])
                    for n in sg.edge_sharing_neighbors(p)]) == 1))
